...
def generate_music(custom_style, bars):
    print(sys.version)
    ...
    if custom_style:
        custom_style = custom_style.split()
        int_styles = [eval(i) for i in custom_style]
        styles = [np.mean([one_hot(i, NUM_STYLES) for i in int_styles], axis=0)]

    write_file('output', generate(models, bars, styles))
